####################################################################################################
### Run intersections between ant range polygons
### Charlie Marsh
### charliem2003@github
### 06/2024
###
### Regions map used: Bioregions_rangemaps
### Taxonomy used:    AntCat
### Rangemaps used:   Kass et al. 2022 (AntMaps)
###
### NOTE: First you must run the scripts (in 'Analysis_functions/Taxon_specific/Ants'):
###       1) 'Generate_ant_rangemaps.R' - this will regenerate the range maps used in Kass et al 2022.
###       2) 'Extract_description_dates_from_antcat.R' - gets description date for tropical Asia species from AntCat
###
### CITATION:
### Kass, J.M., Guénard, B., Dudley, K.L., Jenkins, C.N., Azuma, F., Fisher, B.L., Parr, C.L.,
###   Gibb, H., Longino, J.T., Ward, P.S., Chao, A., Lubertazzi, D., Weiser, M., Jetz, W., Guralnick, R.,
###   Blatrix, R., Lauriers, J.D., Donoso, D.A., Georgiadis, C., Gomez, K., Hawkes, P.G., Johnson, R.A.,
###   Lattke, J.E., MacGown, J.A., Mackay, W., Robson, S., Sanders, N.J., Dunn, R.R., Economo, E.P., 2022.
###   The global distribution of known and undiscovered ant biodiversity. Science Advances 8, eabp9908.
...
rangeDir  <- file.path("Kass", "download", "directory")    # dir that contains the rangemaps data
taxonDir  <- file.path("Kass", "download", "directory")    # dir that contains the taxonomy data (for description year)
...
spDir     <- file.path(resDir,  "Intersections", "Ants")   # dir to save species intersections to
...
### global GADM for masking out coastlines
gadm <- st_read(file.path(gadmDir, "countries", "GADM_410_land_Equal_Area.gpkg"))

### range maps - each species is an individual shapefile
spList <- list.files(file.path(rangeDir, "species_rangemaps"), pattern = ".gpkg")
spList <- gsub(".gpkg", "", spList)

### read in rangemaps
rangemaps <- st_read(file.path(rangeDir, "Kass_alpha_hulls.gpkg"))

### total species list extracted from range maps - 3619 species (12,892 globally)
spList <- rangemaps$Species
...
# filter out completed species (should already be equal-area projection)
rangemaps <- rangemaps %>%
...
intersections_rangemaps_parallel(rangemaps        = rangemaps,    # species range maps
                                 spNameCol        = "Species",    # column name where species names are kept
...
### Taxonomy was extracted from the AntCat API (see 'Extract_description_dates_from_antcat.R')
taxonomy <- read.csv(file.path(taxonDir, "AntCat_taxonomy_for_BTAS_species.csv")) %>%
  select(-Level, -ID, -Status)
...
pa0_0 <- convertToPA(rangeIntersects = intersections,    # data frame of occupied area for species range maps x bioregions
                     ...
                     verbose         = TRUE)
write.csv(pa0_0, file.path(resDir, "Intersections", "Intersections_bioregions_ants_0_0.csv"),
...
pa2.5_15 <- convertToPA(rangeIntersects = intersections, # data frame of occupied area for species range maps x bioregions
                        ...
                        regions         = regions,       # shapefile with bioregions
                        verbose         = TRUE)
write.csv(pa2.5_15, file.path(resDir, "Intersections", "Intersections_bioregions_ants_2.5_15.csv"),
          quote = FALSE, row.names = FALSE)
...
pa5.0_25 <- convertToPA(rangeIntersects = intersections, # data frame of occupied area for species range maps x bioregions
                        ...
                        regions         = regions,       # shapefile with bioregions
                        verbose         = TRUE)
write.csv(pa5.0_25, file.path(resDir, "Intersections", "Intersections_bioregions_ants_5.0_25.csv"),
          quote = FALSE, row.names = FALSE)
...
pa7.5_35 <- convertToPA(rangeIntersects = intersections, # data frame of occupied area for species range maps x bioregions
                        ...
                        regions         = regions,       # shapefile with bioregions
                        verbose         = TRUE)
write.csv(pa7.5_35, file.path(resDir, "Intersections", "Intersections_bioregions_ants_7.5_35.csv"),
          quote = FALSE, row.names = FALSE)
...
write.csv(allSp, file.path(resDir, "Intersections", "Intersections_bioregions_ants_spList.csv"),
          quote = FALSE, row.names = FALSE)
