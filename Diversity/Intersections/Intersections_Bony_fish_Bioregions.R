...
### Note: this isn't comprehensive (although it is for some families). See
### https://www.iucnredlist.org/resources/spatial-data-download for list of families mapped
### Charlie Marsh
### charliem2003@github
### 12/2024
###
### Regions map used: MEOW_BTAS
### Taxonomy used:    IUCN 2024.2 - downloaded 18/11/2024
### Rangemaps used:   IUCN 2024.2 - downloaded 18/11/2024
###
### CITATION: IUCN 2024. The IUCN Red List of Threatened Species. 2024.2.
###   https://www.iucnredlist.org. Downloaded on 18/11/2024.
###
...
### Separate out the bony fish (Actinopterygii) - 9,566 polygons, 8,172 species
rangemaps <- filter(rangemaps, class == "ACTINOPTERYGII")

### Include only marine species - 9,566 polygons, 8,172 species
rangemaps <- filter(rangemaps, marine == "true" | is.na(marine))

### filter out non-relevant range polygons - 9,425 polygons, 8,156 species (species lost are freshwater-breeding eels or extinct)
rangemaps <- rangemaps %>%
...
### total species list extracted from range maps - 8,156 species
...
pa0_0 <- convertToPA(rangeIntersects = intersections,    # data frame of occupied area for species range maps x bioregions
                     ...
                     verbose         = TRUE)
write.csv(pa0_0, file.path(resDir, "Intersections", "Intersections_bioregions_bony_fish_0_0.csv"),
...
pa2.5_15 <- convertToPA(rangeIntersects = intersections, # data frame of occupied area for species range maps x bioregions
                        ...
                        regions         = regions,       # shapefile with bioregions
                        verbose         = TRUE)
write.csv(pa2.5_15, file.path(resDir, "Intersections", "Intersections_bioregions_bony_fish_2.5_15.csv"),
          quote = FALSE, row.names = FALSE)
...
pa5.0_25 <- convertToPA(rangeIntersects = intersections, # data frame of occupied area for species range maps x bioregions
                        ...
                        regions         = regions,       # shapefile with bioregions
                        verbose         = TRUE)
write.csv(pa5.0_25, file.path(resDir, "Intersections", "Intersections_bioregions_bony_fish_5.0_25.csv"),
          quote = FALSE, row.names = FALSE)
...
pa7.5_35 <- convertToPA(rangeIntersects = intersections, # data frame of occupied area for species range maps x bioregions
                        ...
                        regions         = regions,       # shapefile with bioregions
                        verbose         = TRUE)
write.csv(pa7.5_35, file.path(resDir, "Intersections", "Intersections_bioregions_bony_fish_7.5_35.csv"),
          quote = FALSE, row.names = FALSE)
...
write.csv(allSp, file.path(resDir, "Intersections", "Intersections_bioregions_bony_fish_spList.csv"),
          quote = FALSE, row.names = FALSE)
