####################################################################################################
### Cleaning GBIF data for Lycophytes
### Charlie Marsh
### charliem2003@github
### 12/2024
###
...
### Filter just the lycophytes (1,394,390 rows)
gbif <- gbif_par %>%
...
cleaned <- runCleaningSteps(gbif             = gbif,        # prepared gbif data frame, with cols Flag and Flag_step added
                            ...
                            pathFilteredData = file.path(resDir, "gbif_filtered_lycophytes.csv"), # path and file name of gbif data with flagged data filtered out.
                            plot = FALSE)
