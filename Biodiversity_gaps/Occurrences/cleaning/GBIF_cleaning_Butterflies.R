####################################################################################################
### Cleaning GBIF data for Butterflies
### Charlie Marsh
### charliem2003@github
### 12/2024
###
...
### Filter just the butterflies (46,965,049 rows)
gbif <- gbif_par %>%
  filter(family %in% c("Hesperiidae",   #  3,475,791 records
                       "Lycaenidae",    #  7,353,784 records
                       "Nymphalidae",   # 24,718,097 records
                       "Papilionidae",  #  1,763,549 records
                       "Pieridae",      #  9,546,167 records
                       "Riodinidae"     #    107,661 records
  )) %>%
...
cleaned <- runCleaningSteps(gbif             = gbif,        # prepared gbif data frame, with cols Flag and Flag_step added
                            ...
                            pathFilteredData = file.path(resDir, "gbif_filtered_butterflies.csv"), # path and file name of gbif data with flagged data filtered out.
                            plot = FALSE)
