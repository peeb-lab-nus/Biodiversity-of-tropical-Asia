...
### Filter just the bees (11,425,881 rows)
gbif <- gbif_par %>%
  filter(family %in% c("Andrenidae",    # 1,208,507 records
                       "Apidae",        # 6,396,216 records
                       "Colletidae",    #   526,085 records
                       "Halictidae",    # 2,132,549 records
                       "Megachilidae",  # 1,054,793 records
                       "Melittidae",    #   106,004 records
                       "Stenotritidae"  #     1,727 records
  )) %>%
...
cleaned <- runCleaningSteps(gbif             = gbif,        # prepared gbif data frame, with cols Flag and Flag_step added
                            ...
                            pathFilteredData = file.path(resDir, "gbif_filtered_bees.csv"), # path and file name of gbif data with flagged data filtered out.
                            plot = FALSE)
