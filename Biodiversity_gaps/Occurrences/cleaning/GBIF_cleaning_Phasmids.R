####################################################################################################
### Cleaning GBIF data for Phasmids
### Charlie Marsh
### charliem2003@github
### 12/2024
###
...
### Filter just the phasmids (71,252 rows)
gbif <- gbif_par %>%
  filter(order == "Phasmida") %>%
...
cleaned <- runCleaningSteps(gbif             = gbif,        # prepared gbif data frame, with cols Flag and Flag_step added
                            ...
                            pathFilteredData = file.path(resDir, "gbif_filtered_phasmids.csv"), # path and file name of gbif data with flagged data filtered out.
                            plot = FALSE)
