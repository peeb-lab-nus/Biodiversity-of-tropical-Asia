####################################################################################################
### Cleaning GBIF data for Diplopods
### Charlie Marsh
### charliem2003@github
### 12/2024
###
...
### Filter just the diplopods (443,912 rows)
gbif <- gbif_par %>%
  filter(class %in% c("Diplopoda",
                      "Pauropoda",
                      "Symphyla"
                      )) %>%
...
cleaned <- runCleaningSteps(gbif             = gbif,        # prepared gbif data frame, with cols Flag and Flag_step added
                            ...
                            pathFilteredData = file.path(resDir, "gbif_filtered_diplopods.csv"), # path and file name of gbif data with flagged data filtered out.
                            plot = FALSE)
