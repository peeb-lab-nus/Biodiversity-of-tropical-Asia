####################################################################################################
### Cleaning GBIF data for Reptiles
### Charlie Marsh
### charliem2003@github
### 12/2024
###
...
### Filter just the reptiles (11,030,903 rows)
gbif <- gbif_par %>%
  filter(class %in% c("Crocodylia",    #   243,924 records      
                      "Sphenodontia",  #     1,946 records                
                      "Squamata",      # 9,123,851 records
                      "Testudines"     # 1,661,182 records
         )) %>%
...
cleaned <- runCleaningSteps(gbif             = gbif,        # prepared gbif data frame, with cols Flag and Flag_step added
                            ...
                            pathFilteredData = file.path(resDir, "gbif_filtered_reptiles.csv"), # path and file name of gbif data with flagged data filtered out.
                            plot = FALSE)
