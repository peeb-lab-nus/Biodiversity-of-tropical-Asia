####################################################################################################
### Cleaning GBIF data for Gymnosperms
### Charlie Marsh
### charliem2003@github
### 12/2024
###
...
### Filter just the gymnosperms (7,203,564 rows)
gbif <- gbif_par %>%
  filter(class %in% c("Cycadopsida",    #    73,780 records
                      "Gnetopsida",     #    95,088 records
                      "Ginkgoopsida",   #    65,816 records
                      "Pinopsida"       # 6,968,880 records
                      )) %>%
...
cleaned <- runCleaningSteps(gbif             = gbif,        # prepared gbif data frame, with cols Flag and Flag_step added
                            ...
                            pathFilteredData = file.path(resDir, "gbif_filtered_gymnosperms.csv"), # path and file name of gbif data with flagged data filtered out.
                            plot = FALSE)
