####################################################################################################
### Cleaning OBIS data for marine bony fish
### Charlie Marsh
### charliem2003@github
### 12/2024
###
...
### Filter just the bony fish (37,480,780 rows)
obis <- obis_par %>%
  filter(class %in% c(
    ### In worms: Actinopterygii (Gigaclass); Actinopteri (Superclass)
    "Chondrostei", #    262,698 records
    "Holostei",    #     12,045 records
    "Teleostei"    # 37,206,037 records
  )) %>%
...
cleaned <- runCleaningSteps(gbif             = obis,        # prepared obis data frame, with cols Flag and Flag_step added
                            ...
                            pathFilteredData = file.path(resDir, "obis_filtered_bony_fish.csv"), # path and file name of obis data with flagged data filtered out.
                            plot = FALSE)
