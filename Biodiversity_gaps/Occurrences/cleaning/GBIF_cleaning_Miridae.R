####################################################################################################
### Cleaning GBIF data for Miridae
### Charlie Marsh
### charliem2003@github
### 12/2024
###
...
### Filter just the miridae (1,140,414 rows)
gbif <- gbif_par %>%
  filter(family == "Miridae") %>%
...
cleaned <- runCleaningSteps(gbif             = gbif,        # prepared gbif data frame, with cols Flag and Flag_step added
                            ...
                            pathFilteredData = file.path(resDir, "gbif_filtered_miridae.csv"), # path and file name of gbif data with flagged data filtered out.
                            plot = FALSE)
